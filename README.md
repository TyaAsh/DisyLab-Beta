...
> 当前版本为 v0.3.0。项目、画布、资产、历史和 Agent 会话默认保存在本机浏览器中，生成能力需要使用者配置兼容的 API 连接。
...
Agent 会先把需求整理成可编辑的图像方案。用户确认提示词、比例、分辨率、画质、数量和模型后，系统才会创建节点并发起生成。

![Disy Agent 与画布协作](docs/assets/canvas-agent.png)

### 多风格预设

每个预设拥有独立名称、调用词、启用状态和 1–5 张参考图；在 Agent 对话或画布提示词中输入调用词即可引用。预设较多时列表会保持固定高度并滚动。

![Disy 多风格预设](docs/assets/style-presets.png)

### 多画布项目

项目内可以新建、切换和重命名画布；卡片会优先显示最近生成或上传的图片，无图时显示统一占位缩略图。

![Disy 画布切换器](docs/assets/canvas-switcher.png)
