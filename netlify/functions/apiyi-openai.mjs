...
export default async (request) => {
  ...
  const path = url.pathname
    .replace(/^\/apiyi\/openai\/?/, '')
    .replace(/^\/\.netlify\/functions\/apiyi-openai\/?/, '')
    .replace(/^v1(?:beta)?\//i, '')
  ...
  try {
    const upstream = await fetch(target, { method: request.method, headers, body: request.method === 'GET' || request.method === 'HEAD' ? undefined : await request.arrayBuffer() })
